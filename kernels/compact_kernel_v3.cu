...
__global__ void integrate_block(
  ...
  if (local_val.x >= threshold) {
    output[
        (seme[warp_id] + warp_flag - local_cnt + local_out_count + local_idx) + 
        n * ibatch
    ] = local_val.x;
    local_idx++;
  }
...
static cudaError_t compact_v3_workspace_launch(const int* values,
...
cudaError_t compact_v3_launch(const int* values,
  ...
  cudaError_t err = compact_v3_workspace_launch(values,
                                                keep_flags,
                                                tmpput,
                                                tmp_counts,
                                                offset_counts,
