...
__global__ void compact_v2_kernel(const int* values,
                                  int* out_counts,
                                  int batch_size,
                                  int n,
                                  int threshold) {
  ...
  unsigned mask = __ballot_sync(all, flag);
  int warp_cnt = __popc(mask);
...
__global__ void integrate_block(
    const int* input, int* offset_counts, int* output, 
  ...
  {
    output[
        (seme[warp_id] + warp_flag - flag + local_out_count) + 
        n * ibatch
    ] = val;
  }
...
static cudaError_t compact_v2_workspace_launch(const int* values,
  ...
  compact_v2_kernel<block / 32, 32><<<grid, block, 0, stream>>>(
    values, tmp_counts, batch_size, n, threshold
  );
  ...
  integrate_block<block / 32, 32><<<grid, block, 0, stream>>>(
    values, offset_counts, output, n, batch_size, threshold
  );
...
cudaError_t compact_v2_launch(const int* values,
  ...
  cudaError_t err = compact_v2_workspace_launch(values,
                                                keep_flags,
                                                tmpput,
                                                tmp_counts,
                                                offset_counts,
